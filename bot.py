...
JACC_APP_URL          = os.environ.get('JACC_APP_URL', '').strip()
...
# Explicit UI modes prevent ordinary car-price/photo input from being relayed to a proxy chat.
user_modes = {}  # telegram_id(str) -> {"mode": "PROXY_CHAT", "reqId": str}
pending_admin_support = {}  # telegram_id(str) -> {"reqId": str, "role": str}
contact_violation_count = {}  # telegram_id(str) -> int (runtime guard; Sheet logging is attempted)
...
# ── Password Generator ─────────────────────────────────
def _member_expire_date(value):
    """Return expiry date from common Sheet formats, or None."""
...
def _member_user_id(member) -> str:
    return str(member.get("userId") or member.get("userID") or "").replace(".0", "").strip()
...
async def is_active_member(user_id: int) -> bool:
    ...
    try:
        ...
        for m in members:
            if _member_user_id(m) != str(user_id):
                continue
            if str(m.get("status", "")).upper().strip() != "ACTIVE":
                return False
            expire = _member_expire_date(m.get("expireDate") or m.get("expire"))
            return bool(expire and expire >= datetime.now().date())
    except Exception as e:
        logger.error(f"is_active_member: {e}")
    return False
...
def generate_password() -> str:
    ...
    part1 = "".join(secrets.choice(alphabet) for _ in range(6))
    part2 = "".join(secrets.choice(alphabet) for _ in range(4))
    return f"KMT-{part1}-{part2}"
...
async def save_member_to_sheet(user_id: str, username: str, days: int,
    ...
    """Save/update one member and return the actual row details from Sheets.

    The Apps Script currently extends an active member from the existing expiry
    date. Therefore the Bot must not calculate the displayed expiry from today.
    After saving, this function re-reads Members and returns the authoritative
    expireDate/package/password values from the sheet.
    """
    if not SHEET_WEBHOOK:
        return {}
    try:
        async with httpx.AsyncClient(follow_redirects=True) as client:
            ...
            if save_result.get("status") != "ok":
                logger.error(f"saveMember rejected: {save_result}")
                return {}

            # Prefer authoritative values if Apps Script already returns them.
            actual = {
                "status": "ok",
                "expireDate": save_result.get("expireDate") or save_result.get("expiredDate") or "",
                "package": save_result.get("package") or package,
                "password": save_result.get("password") if save_result.get("password") is not None else password,
            }

            # Backward-compatible fallback: read the exact saved member row.
            if not actual["expireDate"]:
                ...
                target_id = str(user_id).replace(".0", "").strip()
                for member in members:
                    member_id = str(
                        member.get("userId")
                        or member.get("userID")
                    ...
                    if member_id != target_id:
                        continue
                    actual["expireDate"] = str(
                        member.get("expireDate")
                        or member.get("expiredDate")
                        or member.get("EXPIREDDATE")
                        or ""
                    ).strip()
                    actual["package"] = str(
                        member.get("package")
                        or member.get("PACKAGE")
                        or package
                    ).strip().upper()
                    sheet_password = member.get("password") or member.get("PASSWORD")
                    if sheet_password is not None:
                        actual["password"] = str(sheet_password)
                    break
...
async def send_approval_dm(context, member_id: int, months: int,
    ...
    is_web = package == "WEB"
    expire_date = actual_expire_date or (
    ...
    if is_web:
        text = (
            ...
            f"📅 သက်တမ်း: *{months} လ*\n"
            f"⏰ စုစုပေါင်းကုန်ဆုံးရက်: `{expire_date}`\n"
            f"ℹ️ ယခင် Membership သက်တမ်းရှိပါက ကျန်ရက်များကို ပေါင်းထည့်ထားပါသည်။\n\n"
            f"🔑 *Web Password*\n`{password}`\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb.append([InlineKeyboardButton("📲 JACC App Download ဆွဲရန်", callback_data="app_download")])
    if ADMIN_USERNAME:
    ...
    if is_admin:
        cmd_text = (
            "*Member Commands:*\n"
            "🚙 `/carrequest` → ကားလိုအပ်ချက်တင်ရန်\n"
            "📋 `/mystatus` → Request Status စစ်ရန်\n"
            "💬 `/brokerchat` → Broker နှင့်စကားပြောရန်\n"
            "📲 `/app` → JACC App Download\n"
...
async def app_download_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if JACC_APP_URL:
        await update.message.reply_text(
            "📲 *JACC App Download*\n\nအောက်က Button ကိုနှိပ်ပြီး Official JACC App ကို Download ဆွဲပါ။",
            parse_mode="Markdown",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("📥 JACC App Download ဆွဲရန်", url=JACC_APP_URL)
            ]]),
        )
        return
    admin_line = f"\n\nApp link ရယူရန် @{ADMIN_USERNAME} ကို ဆက်သွယ်ပါ။" if ADMIN_USERNAME else ""
    await update.message.reply_text("⚠️ JACC App Download link ကို မထည့်ရသေးပါ။" + admin_line)
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    photo_mode = user_modes.get(str_uid, {})
    if photo_mode.get("mode") == "PROXY_CHAT":
        if cust_session_photo and cust_session_photo[0] != photo_mode.get("reqId"):
            cust_session_photo = None
        broker_sessions_photo = [(sid, sess) for sid, sess in broker_sessions_photo if sid == photo_mode.get("reqId")]

    if (cust_session_photo or broker_sessions_photo) and photo_mode.get("mode") == "PROXY_CHAT":
...
# ── Proxy Chat Filter / UI Helpers ────────────────────
def _normalize_proxy_text(text: str) -> tuple[str, str]:
    text = unicodedata.normalize("NFKC", str(text or ""))
    mm_digits = str.maketrans("၀၁၂၃၄၅၆၇၈၉", "0123456789")
    normalized = text.translate(mm_digits).lower()
    compact = re.sub(r"[\s\-_.()\[\]{}/\\|,:;]+", "", normalized)
    return normalized, compact

def proxy_filter(text: str):
    normalized, compact = _normalize_proxy_text(text)

    # Phone-like sequences (Myanmar/Thailand/Malaysia and generic international formats).
    if re.search(r"(?:\+?95|\+?66|\+?60|0)?[1-9](?:[\s\-().]*\d){7,12}", normalized):
        return True, "ဖုန်းနံပါတ် ပေးပို့ခြင်း တားမြစ်ထားသည်"
    if re.search(r"(?:09|959|66|60)\d{7,12}", compact):
        return True, "ဖုန်းနံပါတ် ပေးပို့ခြင်း တားမြစ်ထားသည်"

    # Usernames, emails and links.
    if re.search(r"@[a-z0-9_]{3,}", normalized):
        return True, "Username / Social ID ပေးပို့ခြင်း တားမြစ်ထားသည်"
    if re.search(r"[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}", normalized):
        return True, "Email ပေးပို့ခြင်း တားမြစ်ထားသည်"
    if re.search(r"(?:https?://|www\.|t\.me/|wa\.me/|line\.me/|m\.me/|facebook\.com|fb\.com|fb\.me|instagram\.com|tiktok\.com|wechat\.com)", normalized):
        return True, "Link ပေးပို့ခြင်း တားမြစ်ထားသည်"

    contact_terms = (
        r"whats?app|viber|wechat|we\s*chat|line\s*(?:id)?|messenger|facebook|\bfb\b|"
        r"telegram|signal|zalo|imo|kakao(?:talk)?|instagram|tiktok|contact\s*me|call\s*me|"
        r"inbox\s*me|add\s*me|ဗိုင်ဘာ|ဝက်စ်?အက်ပ်|ဝီချက်|လိုင်း\s*အိုင်ဒီ|"
        r"မက်ဆင်ဂျာ|ဖေ့စ်ဘုတ်|တယ်လီဂရမ်|ဖုန်းဆက်|အင်ဘောက်"
    )
    if re.search(contact_terms, normalized, re.IGNORECASE):
        return True, "Viber / WhatsApp / WeChat / Facebook စသည့် Contact info ပေးပို့ခြင်း တားမြစ်ထားသည်"

    if re.search(r"တိုက်နံပါတ်|အခန်းနံပါတ်|ရပ်ကွက်|မြို့နယ်|တိုင်းဒေသ|ပြည်နယ်|နေရပ်လိပ်စာ|နေထိုင်ရာ", normalized):
        return True, "လိပ်စာ ပေးပို့ခြင်း တားမြစ်ထားသည်"
    if re.search(r"\b(street|road|lane|avenue|address|district|township|quarter)\b", normalized):
        return True, "လိပ်စာ ပေးပို့ခြင်း တားမြစ်ထားသည်"
    return False, ""

def proxy_menu_keyboard(req_id: str, role: str = "customer"):
    rows = [
        [InlineKeyboardButton("💬 Proxy Chat ဝင်ရန်", callback_data=f"proxy_enter_{req_id}")],
        [InlineKeyboardButton("📋 Request အခြေအနေ", callback_data=f"proxy_status_{req_id}"),
         InlineKeyboardButton("🚨 ပြဿနာတင်ပြရန်", callback_data=f"proxy_report_{req_id}")],
        [InlineKeyboardButton("🏠 Main Menu သို့ပြန်ရန်", callback_data=f"proxy_home_{req_id}")],
    ]
    if role == "customer":
        rows.append([InlineKeyboardButton("❌ Car Request ပယ်ဖျက်ရန်", callback_data=f"cancel_prompt_{req_id}")])
    else:
        rows.append([InlineKeyboardButton("🛟 Admin ကိုတင်ပြရန်", callback_data=f"adminhelp_{req_id}")])
    return InlineKeyboardMarkup(rows)

def proxy_active_keyboard(req_id: str, role: str = "customer"):
    rows = [
        [InlineKeyboardButton("🏠 Main Menu သို့ပြန်ရန်", callback_data=f"proxy_home_{req_id}")],
        [InlineKeyboardButton("🚨 ပြဿနာတင်ပြရန်", callback_data=f"proxy_report_{req_id}")],
    ]
...
def customer_chat_notice(req_id: str) -> str:
    return (
        f"💬 *Broker Proxy Chat စတင်ပါပြီ*\n\n🆔 Request: `{req_id}`\n\n"
        "⚠️ *လုံခြုံရေးသတိပေးချက်*\n"
        "• ဖုန်းနံပါတ်၊ Telegram username၊ Viber / WhatsApp / WeChat / LINE ID မပေးပါနှင့်။\n"
        "• Facebook၊ Messenger နှင့် အခြား Social Account မပေးပါနှင့်။\n"
        "• Broker ထံ တိုက်ရိုက်ငွေလွှဲခြင်း မပြုပါနှင့်။\n"
        "• Bot/Admin အတည်ပြုထားသော Account သို့သာ ငွေလွှဲပါ။\n"
        "• Chat စာနှင့်ပုံများကို လုံခြုံရေးအတွက် မှတ်တမ်းတင်ထားပါသည်။\n\n"
        "ယခု Chat Mode အတွင်း ပို့သောစာနှင့်ပုံများကို Broker ထံပို့ပေးမည်။"
    )

def broker_chat_notice(req_id: str) -> str:
    return (
        f"💬 *Customer Proxy Chat စတင်ပါပြီ*\n\n🆔 Request: `{req_id}`\n\n"
        "⚠️ *Broker စည်းကမ်းချက်*\n"
        "• Customer ၏ Phone၊ Viber၊ WhatsApp၊ WeChat၊ LINE၊ Facebook/Messenger ID မတောင်းရပါ။\n"
        "• Platform ပြင်ပ Deal မလုပ်ရပါ။\n"
        "• Bot တွင်မဖော်ပြထားသော ငွေပေးချေမှု မတောင်းရပါ။\n"
        "• ကား၊ Repair History နှင့် Auction Sheet အချက်အလက်ကို မှန်ကန်စွာပြောရပါမည်။\n"
        "• Chat စာနှင့်ပုံများကို Admin စစ်ဆေးနိုင်ရန် မှတ်တမ်းတင်ထားပါသည်။"
    )

async def log_contact_violation(user_id: str, req_id: str, reason: str):
    contact_violation_count[user_id] = contact_violation_count.get(user_id, 0) + 1
    if SHEET_WEBHOOK:
        try:
            async with httpx.AsyncClient(follow_redirects=True) as client:
                await client.post(SHEET_WEBHOOK, json={
                    "action": "saveChatLog", "reqId": req_id, "senderId": user_id,
                    "senderLabel": "SYSTEM_VIOLATION", "msgType": "blocked",
                    "content": reason[:500],
                    "timestamp": datetime.now().strftime("%d/%m/%Y %H:%M:%S"),
                }, timeout=8)
        except Exception as e:
            logger.warning(f"log_contact_violation: {e}")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Broker/Customer -> Admin support message (one-shot).
    support = pending_admin_support.pop(str_uid, None)
    if support:
        req_id = support.get("reqId", "")
        role = support.get("role", "User")
        blocked, reason = proxy_filter(text)
        if blocked:
            await update.message.reply_text(f"🚫 Admin support စာထဲတွင် {reason}")
            return
        await notify_admins(context,
            f"📨 *{role} Support Request*\n\n🆔 `{req_id}`\n👤 Telegram ID: `{str_uid}`\n\n{text}")
        await log_chat_message(req_id, str_uid, f"{role}->Admin", "support", text)
        await update.message.reply_text("✅ Admin ထံ တင်ပြပြီးပါပြီ။ ပြန်ကြားချက်ကို Bot မှတစ်ဆင့်ပို့ပေးမည်။")
        return
    ...
    current_mode = user_modes.get(str_uid, {})

    if cust_session and current_mode.get("mode") == "PROXY_CHAT" and current_mode.get("reqId") == cust_session[0]:
        ...
        blocked, reason = proxy_filter(text)
        if blocked:
            await log_contact_violation(str_uid, session.get("reqId", sid), reason)
            count = contact_violation_count.get(str_uid, 1)
            await update.message.reply_text(
                f"🚫 *Message မပို့နိုင်ပါ*\n\n❌ {reason}\n\n"
                "Phone, Viber, WhatsApp, WeChat, LINE, Telegram, Facebook/Messenger, Link, Email သို့မဟုတ် လိပ်စာကို ဖြုတ်ပြီး ပြန်ပို့ပါ။\n"
                f"⚠️ Warning: {count}", parse_mode='Markdown')
            return
        if broker_tg_id:
            try:
                req_id_c = session.get("reqId", "")
                close_kb = InlineKeyboardMarkup([[
                    InlineKeyboardButton("🏠 Main Menu သို့ပြန်ရန်", callback_data=f"proxy_home_{req_id_c}")
                ]])
                await context.bot.send_message(
                    chat_id=int(broker_tg_id),
    ...
    if broker_sessions and current_mode.get("mode") == "PROXY_CHAT":
        broker_sessions = [(sid, sess) for sid, sess in broker_sessions if sid == current_mode.get("reqId")]
        if not broker_sessions:
            user_modes.pop(str_uid, None)
        else:
            pass
    if broker_sessions and current_mode.get("mode") == "PROXY_CHAT":
        blocked, reason = proxy_filter(text)
        if blocked:
            _rid = broker_sessions[0][1].get("reqId", broker_sessions[0][0])
            await log_contact_violation(str_uid, _rid, reason)
            count = contact_violation_count.get(str_uid, 1)
            await update.message.reply_text(
                f"🚫 *Message မပို့နိုင်ပါ*\n\n❌ {reason}\n\n"
                "Phone, Viber, WhatsApp, WeChat, LINE, Telegram, Facebook/Messenger, Link, Email သို့မဟုတ် လိပ်စာကို ဖြုတ်ပြီး ပြန်ပို့ပါ။\n"
            ...
            if customer_id:
                try:
                    ...
                    close_kb_b  = InlineKeyboardMarkup([[
                        InlineKeyboardButton("🏠 Main Menu သို့ပြန်ရန်", callback_data=f"proxy_home_{req_id_b}")
                    ]])
                    await context.bot.send_message(
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "app_download":
        if JACC_APP_URL:
            ...
            admin_line = f"\n\nApp link ရယူရန် @{ADMIN_USERNAME} ကို ဆက်သွယ်ပါ။" if ADMIN_USERNAME else ""
            await query.message.reply_text("⚠️ JACC App Download link ကို မထည့်ရသေးပါ။" + admin_line)
        return
    ...
    # ── Explicit Proxy Chat Mode / Main Menu ──
    if data.startswith("proxy_enter_"):
        req_id = data.replace("proxy_enter_", "")
        uid = str(query.from_user.id)
        session = proxy_sessions.get(req_id)
        if not session or session.get("status") != "ACTIVE":
            await query.answer("❌ Active Chat မတွေ့ပါ", show_alert=True); return
        is_customer = str(session.get("customerId", "")) == uid
        is_broker = str(session.get("brokerId", "")) == uid
        if not (is_customer or is_broker):
            await query.answer("❌ ဒီ Chat ကို ဝင်ခွင့်မရှိပါ", show_alert=True); return
        if is_customer and req_id.startswith("A") and not session.get("deposit_paid", False):
            await query.answer("⚠️ Deposit Confirm ပြီးမှ Chat ဖွင့်နိုင်ပါမည်", show_alert=True); return
        user_modes[uid] = {"mode": "PROXY_CHAT", "reqId": req_id}
        notice = customer_chat_notice(req_id) if is_customer else broker_chat_notice(req_id)
        await query.message.reply_text(notice, parse_mode="Markdown",
            reply_markup=proxy_active_keyboard(req_id, "customer" if is_customer else "broker"))
        return

    if data.startswith("proxy_home_"):
        req_id = data.replace("proxy_home_", "")
        uid = str(query.from_user.id)
        user_modes.pop(uid, None)
        session = proxy_sessions.get(req_id, {})
        role = "customer" if str(session.get("customerId", "")) == uid else "broker"
        await query.message.reply_text(
            "✅ *Main Menu သို့ ပြန်ရောက်ပါပြီ*\n\n"
            "သင့် Car Request နှင့် Broker Session သည် ဆက်လက်ဖွင့်ထားပါသည်။\n"
            "ပြန်ပြောလိုပါက `💬 Proxy Chat ဝင်ရန်` ကိုနှိပ်ပါ။",
            parse_mode="Markdown", reply_markup=proxy_menu_keyboard(req_id, role))
        return

    if data.startswith("proxy_status_"):
        req_id = data.replace("proxy_status_", "")
        session = proxy_sessions.get(req_id)
        if not session:
            await query.answer("❌ Request မတွေ့ပါ", show_alert=True); return
        await query.message.reply_text(
            f"📋 *Request Status*\n\n🆔 `{req_id}`\n🟢 Broker Session: ACTIVE",
            parse_mode="Markdown")
        return

    if data.startswith("proxy_report_"):
        req_id = data.replace("proxy_report_", "")
        uid = str(query.from_user.id)
        session = proxy_sessions.get(req_id, {})
        role = "Customer" if str(session.get("customerId", "")) == uid else "Broker"
        pending_admin_support[uid] = {"reqId": req_id, "role": role}
        await query.message.reply_text(
            f"🚨 *ပြဿနာတင်ပြရန်*\n\n🆔 `{req_id}`\n\nဖြစ်နေသောပြဿနာကို စာတစ်စောင်တည်းဖြင့် ရေးပို့ပါ။\nAdmin ထံ တိုက်ရိုက်ပို့ပေးမည်။",
            parse_mode="Markdown")
        return

    if data.startswith("adminhelp_"):
        req_id = data.replace("adminhelp_", "")
        uid = str(query.from_user.id)
        session = proxy_sessions.get(req_id, {})
        if str(session.get("brokerId", "")) != uid:
            await query.answer("❌ Broker သာ အသုံးပြုနိုင်ပါသည်", show_alert=True); return
        pending_admin_support[uid] = {"reqId": req_id, "role": "Broker"}
        await query.message.reply_text(
            f"🛟 *Admin Support*\n\n🆔 `{req_id}`\n\nCustomer Request၊ Deposit၊ Auction Sheet သို့မဟုတ် Request ပြန်လွှဲလိုသည့်အကြောင်းကို စာတစ်စောင်တည်းဖြင့် ရေးပို့ပါ။",
            parse_mode="Markdown")
        return

    if data.startswith("cancel_prompt_"):
        req_id = data.replace("cancel_prompt_", "")
        session = proxy_sessions.get(req_id)
        uid = str(query.from_user.id)
        if not session or str(session.get("customerId", "")) != uid:
            await query.answer("❌ ဒီ Request ကို ပယ်ဖျက်ခွင့်မရှိပါ", show_alert=True); return
        kb = InlineKeyboardMarkup([[
            InlineKeyboardButton("✅ ဟုတ်ကဲ့၊ ပယ်ဖျက်မည်", callback_data=f"cancel_confirm_{req_id}"),
            InlineKeyboardButton("⬅️ မပယ်ဖျက်ပါ", callback_data=f"proxy_home_{req_id}"),
        ]])
        await query.message.reply_text(
            f"⚠️ *Car Request ကို တကယ်ပယ်ဖျက်လိုပါသလား?*\n\n🆔 `{req_id}`\n\n"
            "ပယ်ဖျက်ပါက Broker Session ပိတ်သွားပြီး Cancel Count တိုးနိုင်ပါသည်။",
            parse_mode="Markdown", reply_markup=kb)
        return

    if data.startswith("cancel_confirm_"):
        req_id = data.replace("cancel_confirm_", "")
        uid = str(query.from_user.id)
        session = proxy_sessions.get(req_id)
        if not session or str(session.get("customerId", "")) != uid:
            await query.answer("❌ ဒီ Request ကို ပယ်ဖျက်ခွင့်မရှိပါ", show_alert=True); return
        # Deposit-confirmed auction requests cannot be cancelled here.
        if session.get("deposit_paid", False):
            await query.answer("🚫 Deposit Confirm ပြီးသော Request ကို မပယ်ဖျက်နိုင်ပါ", show_alert=True); return
        ...
        try:
            ...
            logger.error(f"cancel_confirm updateRequest: {e}")
        await query.message.reply_text(f"❌ *Car Request ပယ်ဖျက်ပြီးပါပြီ*\n\n🆔 `{req_id}`", parse_mode="Markdown")
        if broker_tg_id:
            try:
                await context.bot.send_message(int(broker_tg_id), f"❌ Customer က Request `{req_id}` ကို ပယ်ဖျက်လိုက်ပါသည်။", parse_mode="Markdown")
            except Exception as e:
                logger.error(f"cancel_confirm broker notify: {e}")
        return
    ...
    if data.startswith("closechat_"):
        ...
        allowed_ids = {str(customer_id or ""), str(broker_tg_id or "")}
        if closer_id not in allowed_ids:
            await query.answer("❌ ဒီ Session ကို ပိတ်ခွင့်မရှိပါ", show_alert=True)
            return
        who = "customer" if closer_id == str(customer_id or "") else "broker"
    ...
    # ── Slip Confirm (Admin) ──
        ...
        saved_member = await save_member_to_sheet(
            str(member_id),
            username.replace("@", ""),
            months * 30,
            password,
        ...
        if not saved_member:
            ...
            if ADMIN_IDS:
                try:
                    await context.bot.send_message(
                        chat_id=ADMIN_IDS[0],
                        text=f"⚠️ Sheet write FAILED for {member_id} — manual fix needed!",
                    )
                except Exception as e:
        ...
        actual_expire_date = str(saved_member.get("expireDate") or "").strip()
        actual_package = str(saved_member.get("package") or package).upper().strip()
        actual_password = str(saved_member.get("password") or password)
        ...
        await send_approval_dm(
            ...
            actual_expire_date=actual_expire_date,
        )

        expire_date = actual_expire_date or (
            datetime.now() + timedelta(days=months * 30)
        ).strftime("%d/%m/%Y")
        pw_line = f"🔑 Password: `{actual_password}`\n" if actual_package == "WEB" else ""

        await query.message.reply_text(
            f"✅ *Payment Confirmed + Approved!*\n\n"
            f"👤 {name} ({username})\n"
            f"📦 {PLAN_NAMES.get(actual_package, actual_package)} — {months} လ\n"
        ...
        if not broker:
            await query.answer("❌ Broker မဟုတ်ဘူး", show_alert=True); return
        active_for_broker = [sess for sess in proxy_sessions.values()
                             if str(sess.get("brokerId", "")) == user_id and sess.get("status") == "ACTIVE"]
        if active_for_broker or str(broker.get("status", "")).upper() in ("BUSY", "FULL", "HAS_AUCTION", "HAS_SEARCH"):
            await query.answer("❌ Active Request ရှိနေသောကြောင့် Request အသစ် မလက်ခံနိုင်ပါ", show_alert=True); return
        ...
        try:
            ...
            if rdata.get("status") == "ok":
                current_status = str(rdata.get("requestStatus") or rdata.get("statusValue") or rdata.get("reqStatus") or "OPEN").upper()
                if current_status not in ("OPEN", "PENDING", "MATCHING", "NEW", ""):
                    await query.answer("❌ ဒီ Request ကို အခြား Broker လက်ခံပြီးပါပြီ", show_alert=True)
                    return
        ...
        proxy_sessions[req_id] = {
            ...
            "reqId":            req_id,
            "status":           "ACTIVE",
            "serviceType":      "auction" if str(req_data.get("carType", "")).lower() == "auction" else "search",
            "startTime":        datetime.now().isoformat(),
        }

        await query.edit_message_text(
            f"✅ *Request လက်ခံပြီ!*\n\n🆔 `{req_id}`\n🚗 {req_data.get('carType','')}\n\n"
            "Customer နဲ့ပြောရန် Proxy Chat ကို ဝင်ပါ။",
            parse_mode='Markdown', reply_markup=proxy_menu_keyboard(req_id, "broker"))
            ...
            try:
                ...
                await context.bot.send_message(
                    ...
                    text=(f"🎉 *Broker ရှာပေးနေပြီ!*\n\n"
                          ...
                          f"{_b_rating_str}\n\n"
                          f"ကားရှာပေးနေတယ် ⏳\n\n"
                          "Broker နှင့်ပြောရန် အောက်ပါ Proxy Chat ကို ဝင်ပါ။"),
                    parse_mode='Markdown', reply_markup=proxy_menu_keyboard(req_id, "customer"))
            except Exception as e:
...
async def approve_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not saved:
        await update.message.reply_text("❌ Google Sheet မှာ Member သိမ်းမရပါ။ Approve မလုပ်ရသေးပါ။")
        return
    ...
    if member_id:
        dm_sent = await send_approval_dm(
            context, member_id, months, password, invite_url, package=package
        )

    expire_date = (datetime.now() + timedelta(days=days)).strftime("%d/%m/%Y")
    dm_status = "✅ Member DM ပို့ပြီး" if dm_sent else "⚠️ DM မပို့နိုင်ပါ — Member ကို /start နှိပ်ခိုင်းပါ"
    txt = (f"✅ <b>Membership Approved!</b>\n\n"
           ...
           f"📨 {dm_status}\n")
    if not invite_url:
        txt += "\n⚠️ Channel invite link ထုတ်မရပါ။ Bot Channel permission စစ်ပါ။"
...
async def available_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    _uid_check = str(update.effective_user.id)
    if any(str(sess.get("brokerId", "")) == _uid_check and sess.get("status") == "ACTIVE" for sess in proxy_sessions.values()):
        await update.message.reply_text("❌ Active Request ရှိနေသေးသောကြောင့် FREE မပြောင်းနိုင်ပါ")
        return
...
# ── /carrequest ─────────────────────────────────────
REQ_STEPS = ["car_name","year","grade","condition","timeline"]
REQ_LABELS = {
    "car_name":  "🚗 ကားအမည်",
...
async def accept_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if rdata.get("status") == "ok":
            current_status = str(rdata.get("requestStatus") or rdata.get("statusValue") or rdata.get("reqStatus") or "OPEN").upper()
            if current_status not in ("OPEN", "PENDING", "MATCHING", "NEW", ""):
                await update.message.reply_text("❌ ဒီ Request ကို အခြား Broker လက်ခံပြီးပါပြီ")
                return
            customer_id       = rdata.get("customerId")
            customer_username = rdata.get("username","")
            req_data          = rdata
    ...
    if active_types:
        await update.message.reply_text("❌ Active Request ရှိနေသောကြောင့် Request အသစ် မလက်ခံနိုင်ပါ")
        return
    ...
    if customer_id:
        try:
            if svc_type == "auction":
                dep_kb = InlineKeyboardMarkup([[
                    InlineKeyboardButton("💰 Deposit ฿20,000 ပေးမည်", callback_data=f"dep_start_{req_id}_{user_id}"),
                    InlineKeyboardButton("💬 Proxy Chat", callback_data=f"proxy_enter_{req_id}"),
                ]])
                customer_text = (f"🎉 *Broker ရှာပြီ — Deposit လိုအပ်ပါသည်*\n\n🆔 `{req_id}`\n"
                                 f"👷 Broker #{broker['brokerId']}\n\n"
                                 "⚠️ Broker ထံ တိုက်ရိုက်ငွေမလွှဲပါနှင့်။ Bot/Admin အတည်ပြုထားသော Account သို့သာ ပေးပါ။")
                await context.bot.send_message(int(customer_id), customer_text, parse_mode="Markdown", reply_markup=dep_kb)
            else:
                await context.bot.send_message(
                    int(customer_id),
                    f"🎉 *Broker ရှာပေးနေပြီ!*\n\n🆔 `{req_id}`\n👷 Broker #{broker['brokerId']}\n\nBroker နှင့်ပြောရန် Proxy Chat ကို ဝင်ပါ။",
                    parse_mode="Markdown", reply_markup=proxy_menu_keyboard(req_id, "customer"))
        except Exception as e:
    ...
    # ── Broker ကို Deposit သတိပေး (Auction only) ──
    if svc_type == "auction":
        nodep_kb = InlineKeyboardMarkup([[
            InlineKeyboardButton(
                "⚠️ Customer Deposit မလွှဲပါ",
        ...
        await update.message.reply_text(
            f"ℹ️ *Deposit သတိပေးချက်*\n\n"
            f"🆔 `{req_id}`\n\n"
            f"Customer အနေနဲ့ လေလံမစခင် 1 နာရီအလိုထိ Deposit ပေးနိုင်ပါသည်\n"
            f"Admin ဆီမှ Deposit Confirm မရသေးရင် Customer ကို သတိပေးပါ\n\n"
            f"Customer က Deposit မလွှဲတော့ဘူးဆိုရင် 👇",
            parse_mode='Markdown', reply_markup=nodep_kb)
...
async def brokerchat_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id)
    sessions = [(sid, sess) for sid, sess in proxy_sessions.items()
                if sess.get("status") == "ACTIVE" and uid in {str(sess.get("customerId", "")), str(sess.get("brokerId", ""))}]
    if not sessions:
        await update.message.reply_text("❌ Active Broker Chat မရှိပါ")
        return
    if len(sessions) > 1 and not context.args:
        rows = [[InlineKeyboardButton(f"💬 {sid}", callback_data=f"proxy_enter_{sid}")] for sid, _ in sessions]
        await update.message.reply_text("ဝင်လိုသည့် Request ကိုရွေးပါ", reply_markup=InlineKeyboardMarkup(rows))
        return
    req_id = context.args[0].upper() if context.args else sessions[0][0]
    sess = proxy_sessions.get(req_id)
    if not sess or uid not in {str(sess.get("customerId", "")), str(sess.get("brokerId", ""))}:
        await update.message.reply_text("❌ ဒီ Chat ကို ဝင်ခွင့်မရှိပါ")
        return
    role = "customer" if str(sess.get("customerId", "")) == uid else "broker"
    user_modes[uid] = {"mode":"PROXY_CHAT", "reqId":req_id}
    notice = customer_chat_notice(req_id) if role == "customer" else broker_chat_notice(req_id)
    await update.message.reply_text(notice, parse_mode="Markdown", reply_markup=proxy_active_keyboard(req_id, role))

async def adminhelp_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = str(update.effective_user.id)
    req_id = context.args[0].upper() if context.args else ""
    if not req_id:
        sessions = [(sid, sess) for sid, sess in proxy_sessions.items() if uid in {str(sess.get("customerId", "")), str(sess.get("brokerId", ""))}]
        if len(sessions) == 1:
            req_id = sessions[0][0]
        else:
            await update.message.reply_text("❌ Format: /adminhelp R123456")
            return
    sess = proxy_sessions.get(req_id, {})
    role = "Broker" if str(sess.get("brokerId", "")) == uid else "Customer"
    pending_admin_support[uid] = {"reqId":req_id, "role":role}
    await update.message.reply_text(f"🛟 `{req_id}` အတွက် Admin ထံတင်ပြမည့် စာကို ယခုပို့ပါ။", parse_mode="Markdown")
...
async def joinchannel_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Issue a fresh, one-use Channel invite link to an active member."""
    user_id = update.effective_user.id
    if not await is_active_member(user_id):
        await update.message.reply_text(
            "❌ Active Membership မရှိပါ။ Membership ဝယ်ရန် /start သို့ သက်တမ်းတိုးရန် /renew ကိုနှိပ်ပါ။"
        )
        return
    ...
    if not invite_url:
        await update.message.reply_text(
            "❌ Channel invite link ထုတ်မရပါ။ Admin ကိုဆက်သွယ်ပါ။"
        )
        return

    keyboard = [[InlineKeyboardButton("📢 Channel ဝင်ရန်", url=invite_url)]]
    await update.message.reply_text(
        "✅ Channel invite link အသစ် ထုတ်ပေးပြီးပါပြီ။\n\n"
        "ဒီ link ကို 24 နာရီအတွင်း တစ်ကြိမ်သာ အသုံးပြုနိုင်ပါတယ်။",
...
async def is_valid_member(user_id: int) -> bool:
    """Return True only when Sheet shows ACTIVE and the expiry date is valid."""
    if user_id in ADMIN_IDS:
        return True
    try:
        async with httpx.AsyncClient(follow_redirects=True) as client:
        ...
        return True  # Backend temporary error: do not kick a possibly valid member.
...
async def main():
    ...
    app.add_handler(CommandHandler("app",         app_download_cmd))
    ...
    app.add_handler(CommandHandler("joinchannel", joinchannel_cmd))
    ...
    app.add_handler(CommandHandler("brokerchat",     brokerchat_cmd))
    ...
    member_commands = [
        ...
        BotCommand("brokerchat",    "💬 Broker Proxy Chat ဝင်ရန်"),
        BotCommand("adminhelp",     "🛟 Admin ထံတင်ပြရန်"),
        BotCommand("app",           "📲 JACC App Download ဆွဲရန်"),
        BotCommand("renew",         "🔄 Membership သက်တမ်းတိုး"),
        BotCommand("mypassword",    "🔑 Password ပြန်ယူရန်"),
        BotCommand("joinchannel",   "📢 Channel link အသစ်ယူရန်"),
