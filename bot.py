...
async def save_member_to_sheet(user_id: str, username: str, days: int,
    ...
    clean_package = str(package or "CH").upper().strip()
    if clean_package in ("WEB-PROMO", "WEB_PREMIUM"):
        clean_package = "WEB"
    elif clean_package in ("CH-PROMO", "STANDARD"):
        clean_package = "CH"
...
async def send_approval_dm(context, member_id: int, months: int,
    ...
    if is_web:
        text = (
            ...
            f"🔑 Web Password ကို နောက် message တစ်ခုထဲ သီးသန့်ပို့ထားပါတယ်။\n"
            f"Password message ကို ဖိထားပြီး Copy လုပ်ပါ။\n\n"
            ...
            f"Channel link ပျောက်သွားရင် /channel ကိုနှိပ်ပါ။\n"
    ...
    try:
        ...
        # Send the password as a password-only plain-text message.
        # This prevents copied values from containing labels, quotes or spaces.
        if is_web and str(password or "").strip():
            await context.bot.send_message(
                chat_id=member_id,
...
async def mypassword_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data.get("status") == "ok" and password:
            await update.message.reply_text(
                "🔑 *သင်၏ Web Password*\n\n"
                "Password ကို နောက် message တစ်ခုထဲ သီးသန့်ပို့ထားပါတယ်။\n"
                "အဲဒီ message ကို ဖိထားပြီး Copy လုပ်ပါ။\n\n"
...
async def channel_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Give an active member a fresh one-use channel invite link."""
    user = update.effective_user
    user_id = user.id

    if not await is_active_member(user_id):
        await update.message.reply_text(
            "🔒 *Active Member မဟုတ်သေးပါ*\n\n"
            "Membership ရယူရန် /start ကိုနှိပ်ပါ။",
            parse_mode="Markdown",
        )
        return
    ...
    try:
        ...
        if member_info.status in ("member", "subscriber", "administrator", "creator"):
            await update.message.reply_text("✅ Channel ထဲ ဝင်ထားပြီးသားပါ။")
            return
    except Exception as e:
        # A missing/non-member user can raise here depending on Telegram response.
        logger.info(f"channel membership lookup {user_id}: {e}")
    ...
    if not invite_url:
        ...
        await update.message.reply_text(
            "❌ Channel invite link မထုတ်နိုင်သေးပါ။ Admin ကို ဆက်သွယ်ပါ။"
            f"{admin_link}"
        )
        return
    ...
    await update.message.reply_text(
        "✅ *Channel Invite Link အသစ်*\n\n"
        "ဒီ link က လူတစ်ယောက်သာ သုံးနိုင်ပြီး 30 မိနစ်အတွင်း ဝင်ရပါမယ်။",
        parse_mode="Markdown",
...
async def main():
    ...
    member_commands = [
        ...
        BotCommand("channel",       "📢 Channel link ပြန်ယူရန်"),
