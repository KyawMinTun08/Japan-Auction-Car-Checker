...
class JaccPhase1Error(RuntimeError):
    """Raised when a Phase 1 RPC or lookup fails."""
...
class JaccPhase1Client:
    ...
    async def get_profile_by_telegram_user_id(
        self,
        telegram_user_id: int,
    ) -> dict[str, Any]:
        url = f"{self._base_url}/rest/v1/jacc_profiles"
        params = {
        ...
        if response.is_error:
            detail = response.text[:1_000]
            raise JaccPhase1Error(
                f"Profile lookup failed ({response.status_code}): {detail}"
            )
