...
(function (global) {
  ...
  function detectClientApp(locationObject, navigatorObject) {
    ...
    let requested = '';
    try {
      requested = new URLSearchParams(locationValue.search || '')
        .get('app') || '';
    } catch (error) {
      requested = '';
    }
    ...
    if (
      app === 'flutter' ||
      global.JACC_FLUTTER_APP === true ||
      userAgent.indexOf('jaccflutter') >= 0
    ) {
      return 'flutter';
    }
    if (
  ...
  function withDevice(action, fields, options) {
    ...
    if (DEVICE_ACTIONS.has(String(action || ''))) {
      Object.assign(payload, deviceFields(options));
    }
    return payload;
  }
  ...
  function reasonOf(response) {
    return String(
      (response && (response.message || response.msg || response.error)) || ''
    ).trim().toLowerCase();
  }
  ...
  function deviceErrorMessage(response) {
    ...
    if (reason === 'device_mismatch') {
      return '🔒 ဒီ Membership ကို အခြား device တစ်ခုနဲ့ ချိတ်ထားပါတယ်။ Telegram Bot မှ Admin ကို Device Reset တောင်းပါ။';
    }
    if (reason === 'device_required' || reason === 'device_invalid') {
      return '⚠️ ဒီ browser/app မှာ secure Device ID မဖန်တီးနိုင်ပါ။ Private mode ပိတ်ပြီး ပြန်ဖွင့်ပါ။';
    }
    if (reason === 'device_lock_busy') {
      return '⏳ Device စစ်ဆေးမှု အလုပ်များနေပါတယ်။ ခဏနေရင် ပြန်ကြိုးစားပါ။';
    }
  ...
  async function verifyStoredSession(options) {
    ...
    if (!result || String(result.status || '').toLowerCase() !== 'ok') {
      ...
      return {
        ok: false,
        reason: reasonOf(result) || 'token_rejected',
        deviceError: isDeviceError(result),
        response: result,
      };
    }
