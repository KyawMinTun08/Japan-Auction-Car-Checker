...
def _server_key() -> str:
    """Read the existing Railway Apps Script credential without logging it."""
    return str(
...
async def resetdevice_cmd(update, context):
    ...
    if not _server_key():
        await message.reply_text(
            "❌ Device Reset authorization key မရှိပါ\n"
            "Railway JACC_SERVER_KEY / SHEET_SERVER_KEY ကို စစ်ပါ။"
        )
        return
    ...
    if result_message in {"unauthorized", "server_key_not_configured"}:
        await message.reply_text(
            ...
            "Railway server key နဲ့ Apps Script JACC_SERVER_KEY တူ/မတူ စစ်ပါ။",
            parse_mode="Markdown",
        )
        return
